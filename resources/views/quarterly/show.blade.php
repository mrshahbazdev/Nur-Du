    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ $quarterlyFocus->quarter }} {{ $quarterlyFocus->year }} &mdash; Strategic Priorities
            </h2>
            <a href="{{ route('quarterly.index') }}" class="text-sm text-indigo-600 hover:text-indigo-800">&larr; Back</a>
        </div>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
                                    ...
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1">
                                            ...
                                            <div class="mt-2 flex flex-wrap gap-4 text-sm text-gray-500">
                                                @if($priority->owner)
                                                    <span>Owner: <strong>{{ $priority->owner }}</strong></span>
                                                @endif
                                                @if($priority->kpi)
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div>
                                                ...
                                                <select name="status" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                                    <option value="on_track" {{ $priority->status === 'on_track' ? 'selected' : '' }}>On Track</option>
                                                    <option value="at_risk" {{ $priority->status === 'at_risk' ? 'selected' : '' }}>At Risk</option>
                                                    <option value="off_track" {{ $priority->status === 'off_track' ? 'selected' : '' }}>Off Track</option>
                                                </select>
                                            </div>
            ...
            @if($quarterlyFocus->strategicPriorities->count() < 3)
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">Add Strategic Priority</h3>
                        <p class="text-sm text-gray-500 mb-4">Define 1&ndash;3 strategic priorities that clearly contribute to your vision.</p>
                        <form method="POST" action="{{ route('quarterly.priorities.store', $quarterlyFocus) }}">
                            @csrf
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Priority Title</label>
                            ...
                            <div class="mt-4">
                                <x-primary-button>Add Priority</x-primary-button>
                            </div>
                        </form>
                    </div>
                </div>
            @else
                <div class="bg-yellow-50 border border-yellow-200 text-yellow-700 px-4 py-3 rounded">
                    Maximum of 3 strategic priorities reached for this quarter.
                </div>
            @endif
